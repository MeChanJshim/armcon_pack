...
int main(int argc, char** argv)
{
    ...
    const std::string package_path = config.package_bundle_dir + "/Y2RobMotion";
    const std::string load_file = (config.trajectory_mode == 0)
        ? "txtcmd/cmd_6D.txt"
        : ((config.trajectory_mode == 1) ? "txtcmd/cmd_9D.txt" : "txtcmd/cmd_continue9D.txt");
    ...
    if(load_file=="txtcmd/cmd_6D.txt")
    {
        pg_param.loadFileType = "cmd_6D";
        printf("\033[33mcmd_6D was selected \033[0m\n");
    }
    else if(load_file == "txtcmd/cmd_9D.txt") {
        pg_param.loadFileType = "cmd_9D";
        printf("\033[33mcmd_9D was selected \033[0m\n");
    }
    else if(load_file == "txtcmd/cmd_continue9D.txt") {
        pg_param.loadFileType = "cmd_continue9D";
        printf("\033[33mcmd_continue9D was selected \033[0m\n");
    }
    else {printf("\033[31mWrong file was selected \033[0m\n");}
    
    printf("\033[33mCONTROL_PERIOD_: %dms \033[0m\n",static_cast<int>(config.control_period*1000));

    /* Node hip instance generation */ 
    std::string Node_name = "singleArm_cmd";
    auto cmd_node = std::make_shared<singleArm_cmd>(
        Node_name,
        config.robot_name,
    ...
    while (rclcpp::ok()) {
        if (cmd_node->is_roboInit()) {
            std::cout << "Enter command mode (Idling(i), PTP(p), TxtLoad(t), Guiding(g), 9999:Quit): ";
